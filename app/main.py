...
app = FastAPI(
    ...
    version="1.0.0",
    description="API that calculates commute weather risk and suggests the best departure time."
)
...
def commute_advice(request: CommuteRequest):
    """
    Main API endpoint:
    - Fetch weather forecast + caching status
    - Fetch AQI
    - Find best departure window
    - Return risk score + breakdown + recommendation
    """


    forecast, cache_status = fetch_hourly_forecast(
    ...
    if best_dt == planned_dt:
        ...
        message = f"Better conditions expected if you leave at {best_option['departure'].strftime('%H:%M')}"
